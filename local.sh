...
if ! [ -f .template/data/priv_validator_state.json ]; then
	$SIMD init validator --chain-id "template-1" --home $HOME_DIR &>/dev/null

	$SIMD keys add validator --home .template --keyring-backend test &>/dev/null
	...
	$SIMD genesis gentx validator 1000000ustake --chain-id "template-1" --home $HOME_DIR --keyring-backend test &>/dev/null
